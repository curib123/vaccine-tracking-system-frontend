...
import GenerateVaccineModal from "@/components/modal/GenerateVaccineModal"; // 🆕
...
function ChildrenPageContent() {
  ...
  const [generateChildId, setGenerateChildId] = useState<number | null>(null);
          ...
          <tbody>
            ...
            {children.map((c) => (
              <tr key={c.id} className="hover:bg-slate-50 transition">
                ...
                <td className="px-6 py-4 text-slate-500">{c.gender}</td>

                <td className="px-6 py-4 text-slate-500">
                  {new Date(c.birthDate).toLocaleDateString()}
                </td>

                <td className="px-6 py-4 text-slate-500">{c.birthPlace}</td>
